...
export class TextDisplayComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    // Subscribe to INPUT text changes (for the textarea)
    this.subscriptions.add(
      this.textService.inputText$.subscribe(text => {
        this.inputText = text;
      })
    );
    
    // Subscribe to OUTPUT text changes (for display)
    this.subscriptions.add(
  ...
  onTextInput(event: any): void {
    ...
    this.textService.updateInputText(newText);
  }
